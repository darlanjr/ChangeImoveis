...
class PHPFrodo
{
    ...
    /**
     *
     * Utilizado para realizar seleção com a condição OR (ou)
     *
     * @name orwhere
     * @param String $condition
    ...
    public function orwhere( $condition = null )
    {
        try
        {
            if ( $condition == null )
            {
                throw new Exception( "orwhere: A condição deve ser informada como parâmetro do método." );
            }
            else
            {
    ...
    /**
     *
     * Utilizado para realizar seleção com a condição AND (ou)
     *
     * @name andwhere
     * @param String $condition
    ...
    public function andwhere( $condition = null )
    {
        try
        {
            if ( $condition == null )
            {
                throw new Exception( "andwhere: A condição deve ser informada como parâmetro do método." );
            }
            else
            {
    ...
    /**
     *
     * Utilizado para realizar seleção com a condição Like em strings
     * @name like
    ...
    public function like( $field = null, $value = null )
    {
        try
        {
            ...
            {
                throw new Exception( "like: O campo da tabela e valor comparado devem ser informados como parâmetro do método." );
            }
            else
            {
    ...
    public function orlike( $field = null, $value = null )
    {
        try
        {
            if ( $field == null || $value == null )
            {
                throw new Exception( "orlike: O campo da tabela e valor comparado devem ser informados como parâmetros do método." );
            }
            else
            {
    ...
    /**
     * Utilizado para função explod
     * @name explod
     * @param String $pattern
     * @param String $key
     * @param String $idx
     * @example $obj->preg('-','key_data_cad',0);
     *
     */
    public function explod( $pattern, $key, $index )
    {
        try
        {
            if ( !empty( $this->data ) )
            {
                foreach ( $this->data as $idx => $item )
                {
                    if ( isset( $item[trim( $key )] ) )
                    {
                        $this->data[$idx][trim( $key )] = explode( $pattern, $this->data[$idx][trim( $key )] );
            ...
            {
                $this->response = "explod: O array de origem está vazio.";
                //throw  new Exception("explod: O array de origem está vazio.");
            }
        }
        catch ( Exception $e )
        {
    ...
    public function mask( $key, $mask )
    {
        try
        {
            ...
            {
                $this->response = "mask: O array de origem está vazio.";
                //throw  new Exception("mask: O array de origem está vazio.");
            }
        }
        catch ( Exception $e )
        {
     ...
     */
    public function money( $tbkey = null )
    {
        try
        {
            if ( !empty( $this->data ) )
            {
                foreach ( $this->data as $idx => $val )
                {
                    ...
                    {
                        $this->data[$idx]["$tbkey"] = @number_format( $this->data[$idx]["$tbkey"], 2, ',', '.' );
                    }
                }
            }
            else
    ...
    public function postGetValue( $index )
    {
        ...
        if ( isset( $this->post_fields["$value"] ) )
        {
            return trim( $this->post_values[$value] );
        }
        else
        {
            return false;
    ...
    public function postBlankDrop()
    {
        ...
        {
            $remove = @array_search( $index, $this->post_fields );
            if ( $this->post_values[$remove] == "" || empty( $this->post_values[$remove] ) )
            {
                unset( $this->post_fields[$remove] );
                unset( $this->post_values[$remove] );
            }
    ...
    public function scaffold( $conf = array( 'table' => null, 'bd' => null, 'url' => '', 'keys' => null ) )
    {
        try
        {
            echo "<link href=\"$this->baseUri/app/css/default/scaff.css\" rel=\"stylesheet\" type=\"text/css\" />\n";
            echo "<div id=\"scaff\">\n";
            if ( $conf['table'] == null )
            {
                $bd = $conf['bd'];
                ...
                $this->query = "select * FROM information_schema.tables WHERE  TABLE_SCHEMA = '$bd'";
                $this->method = "select";
                $this->execute();
                $data = $this->data;
            ...
            else
            {
                ...
                $this->query = "select * FROM information_schema.columns WHERE table_name = '$table' and TABLE_SCHEMA = '$bd'";
                $this->method = "select";
                $this->execute();
                $data = $this->data;
                ...
                {
                    $id = $_GET['delete'];
                    $this->delete()->from( "$table" )->where( "$pkey = '$id'" )->execute();
                    $this->redirect( "$url/$table/" );
                }

                $this->select()->from( "$table" )->execute();
                if ( $this->result() )
                {
                    foreach ( $this->data as $item )
                    {
                        ...
                        echo "<td><a href=\"?delete=$pkeyv\">delete</a> ";
                        echo "<a href=\"?edit=$pkeyv\">edit</a></td>";
                        echo "</tr>\n";
                    }
                }
                echo "</table>\n";
                ...
                if ( isset( $_GET['edit'] ) && !empty( $_GET['edit'] ) )
                {
                    ...
                    $id = $_GET['edit'];
                    $this->select()->from( "$table" )->where( "$pkey = '$id'" )->execute();
                    if ( $this->result() )
                    {
                        $data = $this->data[0];
                        $scaff = '<form name="f" action="?update=' . $id . '" method="post">' . "\n";
                        ...
                        $scaff .= '<br /><br /><button>Atualizar Registro</button>' . "\n";
                        $scaff .= '</form>' . "\n";
                        echo $scaff;
                        exit;
                    }
                ...
                $scaff .= '<br /><br /><button>Incluir Registro</button>' . "\n";
                $scaff .= '</form>' . "\n";
                echo $scaff;
            }
            echo "</div>\n";
